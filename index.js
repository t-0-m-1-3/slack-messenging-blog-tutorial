...
app.get('/auth/slack/callback',
  ...
  (req, res) => {
    res.send('<p>Bot was successfully installed on your team.</p>');
  },
  (err, req, res, next) => {
    res.status(500).send(`<p>Bot failed to install</p> <pre>${err}</pre>`);
  }
);
...
console.log('starting the slackEvents.on function');
...
console.log('inside the first if statement');
	...
	console.log('entering into the slack.chat.postMessage')
